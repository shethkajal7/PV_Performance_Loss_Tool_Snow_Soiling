...
How to Use the PV Snow & Soiling Loss Calculator
Step 1 - Open the Tool
Go to the web address above in any modern browser (Chrome, Firefox, Edge, Safari).
You will see a page titled Solar PV Performance Loss Modelling Application: Snow & Soiling (Monthly).
...
Run model
This tells the tool to compute:
•	Snow loss (%)
•	Soiling loss (%)
•	Final (maximum) loss (%)
...
You will see:
•	A) A Table
Shows monthly losses from:
•	Snow
•	Soiling
•	Final dominant effect
•	B) A Chart
A stacked bar chart showing how snow and soiling contribute to total monthly losses.
This helps you visually see when losses are highest in the year.

Step 7 - Download Results
Below the chart, you can download:
•	Results CSV - monthly loss percentages
•	Inputs Used CSV - exactly what the tool used for calculations (handy for reporting or reproducibility)
